...
function AllGamesTab({ games }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'playoffs') return sorted.filter(g => g.gameType === 'MLS Cup Playoffs')
  ...
  return (
    <>
      <div className="flex gap-1 mb-4 rounded-xl p-1 w-fit" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
        {GAME_FILTERS.map(f => (
    ...
    <div className="p-4 md:p-6">
      <div className="flex items-center gap-4 mb-7">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Inter Miami CF regular season + MLS Cup Playoffs</p>
        </div>
      </div>
